...
function Managecourses() {
    ...
    const handleDeleteClick=async(course)=>{
      try{
        const response = await basepath.delete(`/daewaandirshad/deletecourse/${course.course_id}`);
        console.log(response);
    ...
    const handleOpenCourse=async(course)=>{
      ...
      try{
        const response= await basepath.put(`/daewaandirshad/opencourse/${course.course_id}`,{status:newStatus})
        if(response.status == 200){
          setCourses(
            courses.map((c)=>c.course_id===course.course_id?{...c,open_for_registration:newStatus}:c)
          )
        }
        console.log(response);
      ...
      {courses.map((course)=>(
         ...
         <div className="featured-item ">
           ...
           <div className="featured-desc">
             ...
             <span style={{fontFamily:'sans-serif'}}> { course.course_unique_id}</span></h6>
             <h6><span style={{fontWeight:"bold"}}>course description: </span> <span className='' style={{fontFamily:'sans-serif'}}>{course.course_description}</span></h6>
             <h6 > <span style={{fontWeight:"bold"}}>Author : </span> <span style={{fontFamily:'sans-serif'}}>{course.course_writter}</span></h6>
             <h6> <span style={{fontWeight:"bold"}}>ustaz:</span> <span style={{fontFamily:'sans-serif'}}>{course.ustaz}</span></h6>
           </div>
         ...
         <button className={course.open_for_registration==1?"btn btn-danger  p-4 fs-8" :"btn btn-success  p-4 fs-8" } style={{width:"100%",borderRadius:"0",border:"none",color:"white"}} onClick={()=>handleOpenCourse(course)}>{course.open_for_registration==1?"close course":"open course"}</button>
