...
+(($) => {
	...
	var Validator = function (element, options) {
		...
		this.$element.find("[data-match]").each(function () {
			...
			$(target).on("input.bs.validator", (e) => {
	...
	Validator.prototype.onInput = function (e) {
		...
		this.validateInput($el, deferErrors).done(() => {
			this.toggleSubmit();
		});
	};
	...
	Validator.prototype.validateInput = function ($el, deferErrors) {
		...
		return this.runValidators($el).done((errors) => {
			$el.data("bs.validator.errors", errors);

			errors.length
				? deferErrors
					? this.defer($el, this.showErrors)
					: this.showErrors($el)
				: this.clearErrors($el);
	...
	Validator.prototype.showErrors = function ($el) {
		...
		errors = $("<ul/>")
			...
			.append(
				$.map(errors, (error) => $("<li/>")[method](error)),
			);
